...
fix_ASV <- read.csv(here::here("Processed_data", #read in fixed ASV-species combo
                                           ...
                                           "12s_e",
                                           "LITassignment",
                                           "problemASV_edited.csv")) #added missing ASVs from end of script
...
q6$LCT <- c("Anoplarchus insignis/purpurescens", "Sebastes caurinus/maliger", 'Lepidopsetta bilineata/polyxystra', 'Xiphister atropurpureus/mucosus', 
...
#searched each of these ASVs in the out of range dataframe (from Fasta files) into NCBI to determine most probable species
...
#ASV397- Allosmerus elongatus 
#ASV541- Gadus macrocephalus
#ASV566 - Atheresthes stomias 
...
y2 <- y1 %>%                              #species outside of range, assign taxa by next best hit
  ...
  mutate(LCT = c("Salvelinus malma", "Allosmerus elongatus", " Lycodapus mandibularis", "Salvelinus malma", "Clupea palassi",
                 "Atheresthes stomias", "Allosmerus elongatus", "Gadus macrocephalus", "Atheresthes stomias", "Gadus macrocephalus",
...
length(data$ASV) #469
length(unique(data$ASV)) #469
length(unique(top10_occ$ASV)) #469
...
fix_ASV <- read.csv(here::here("Processed_data",
                               ...
                               "12s_u",
                               "LITassignment",
                               "problemASV_edited.csv")) #added ASV 218
...
#ASV292- Gadus macrocephalus
#ASV34- Gadus macrocephalus
#ASV353 - Lampanyctus fernae
#ASV364 - Gadus macrocephalus
#ASV425 - Atheresthes stomias 
#ASV434 - Clupea pallasii
#ASV515 - Gadus macrocephalus
#ASV607 - Gadus macrocephalus
#ASV609 - Salvelinus malma
#ASV65 - Atheresthes stomias 


####
y2 <- y1 %>%                              #species outside of range, assign taxa by next best hit
  filter(in_range != "y") %>%
  mutate(LCT = c("Lycodapus mandibularis", "Gadus macrocephalus", "Gadus macrocephalus", "Lampanyctus fernae", " Gadus macrocephalus",
                 "Atheresthes stomias", "Clupea pallasii", "Gadus macrocephalus", "Gadus macrocephalus",  "Salvelinus malma",
                 "Atheresthes stomias"))%>%             #new assignment goes here (between "")
  mutate(level = c("species")) %>%             #level of assignment goes here (between "") 
  mutate(in_range = c("y"))%>%         
  mutate(all_species = LCT)%>%         
  mutate(species = LCT)
...
#check that all ASVs are accounted for and not duplicated - they should all be the same
length(data$ASV) #476
length(unique(data$ASV)) #476
length(unique(top10_occ$ASV)) #476
