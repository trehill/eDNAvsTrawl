...
syn$standard_length_cm <- syn$standard_length_mm/10 #convert mm to cm by creating a new column 
...
syn <- select(syn, c('Scientific.name','standard_length_cm','fork_length_cm', 'total_length_cm', 'Specimen.identifer'))
...
syn <- data.frame(lapply(syn, function(x) {
  gsub("Anoplarchus insignis","Anoplarchus insignis/purpurescens", x) }))

syn <- data.frame(lapply(syn, function(x) {
  gsub("Anoplarchus purpurescens","Anoplarchus insignis/purpurescens", x) }))

syn <- data.frame(lapply(syn, function(x) {
  gsub("Lepidopsetta bilineata","Lepidopsetta bilineata/polyxystra", x) }))

syn <- data.frame(lapply(syn, function(x) {
  gsub("Lepidopsetta polyxystra","Lepidopsetta bilineata/polyxystra", x) }))

syn <- data.frame(lapply(syn, function(x) {
  gsub("Liparis dennyi","Liparis dennyi/fucensis", x) }))

syn <- data.frame(lapply(syn, function(x) {
  gsub("Liparis fucensis","Liparis dennyi/fucensis", x) }))

syn <- data.frame(lapply(syn, function(x) {
  gsub("Pholis laeta","Pholis laeta/ornata", x) }))

syn <- data.frame(lapply(syn, function(x) {
  gsub("Pholis ornata","Pholis laeta/ornata", x) }))
...
#missing species: 30 missing (over half- we should just use max length)
...
length <- select(length, c('Scientific.name', 'fork_to_total_length_cm', 'Specimen.identifer'))
combined <- merge(data, length, by=c('Scientific.name', 'Specimen.identifer'), all.x=TRUE)
