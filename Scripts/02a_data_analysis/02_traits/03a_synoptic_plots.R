...
trait_data <- read.csv(here::here("Processed_data", 
                                  "datasets",
                                  "traits",
                                  "traits_mean_lengths_A.csv"), #original trait database w/ added mean lengths 
                       head=TRUE)
...
ggsave("./Outputs/analysis_a/traits/syn_length/synop_eDNAtrawl_density.png", 
       plot = plot,
...
ggsave("./Outputs/analysis_a/traits/syn_length/all_length_dist_synop.png", 
       plot = plot,
       width = 10, height = 6, units = "in")
...
ggsave("./Outputs/analysis_a/traits/syn_length/trawl_edna_2_synop.png", 
       plot = plot,
...
ggsave("./Outputs/analysis_a/traits/syn_length/max_length_density.png", 
       plot = plot,
       width = 12, height = 7, units = "in")
...
with(speciestraits, shapiro.test(total_mean_length_cm[method == "eDNA"])) #p = 5.732e-11(not normal)

# Shapiro-Wilk normality test for trawl length
with(speciestraits, shapiro.test(total_mean_length_cm[method == "trawl"])) #p=0.02873 (not normal)
