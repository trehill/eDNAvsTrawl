...
fixnames <- data.frame(lapply(fixnames, function(x) {
  gsub("Sebastes flavidus", "Sebastes", x) #for some reason this species name does not match across datasets
...
fixnames <- data.frame(lapply(fixnames, function(x) {
  gsub("Pacific Hake (Whiting)", "Pacific Hake", x) 
...
fixnames <- data.frame(lapply(fixnames, function(x) {
  gsub("Unidentified Rockfish", "Sebastes", x) 
...
#remove Corphaenidoes sp
#we decided that these spp. appear only once and don't have any measurements (length/weight)
