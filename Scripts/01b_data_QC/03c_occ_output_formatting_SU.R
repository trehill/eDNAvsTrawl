...
out <- read.csv("./Scripts/occupancy_modelling/royle_link/scratch/occProb_royallink_u.csv")
rownames(out) <- out$X
out$X <- NULL
occprob <- out  
...
write.csv(occprob_by_sample, "./Outputs/occupancy_modelling/royle_link/occprob_by_sample_u.csv")
...
write.csv(ASV_LowOccRemoved, "./Scripts/occupancy_modelling/royle_link/scratch/data12se_asvmatrix_lor_12s_u.csv")

#another file path to export data to

write.csv(ASV_LowOccRemoved, "./Processed_data/eDNA/12s/12s_u/asv/matrix/data12su_asvmatrix_nc_lor.csv")
