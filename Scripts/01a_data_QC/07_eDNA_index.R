...
eDNAmeta <- read_csv(here("Processed_data",
                          "eDNA",
                          "metadata",
                          "eDNA_metadata.csv"))
...
data12se_asvmatrix_nc_lor_nfc <- read_csv(here("Processed_data",
  ...
  rename("sample_name" = "original_sample_name")
...
#Format data
#add ASVs across PCR replicates
#aggregate data to sum ASVs across PCR replicates
data12se_asvmatrix_nc_lor_nfc <- data12se_asvmatrix_nc_lor_nfc %>%
  group_by(sample_name) %>%
  summarise(across(starts_with("ASV"), sum, na.rm = TRUE))

#combine sample_name (name of PCR rep) to original sample name
sample_info <- select(eDNAmeta, c('sample_name', 'original_sample_name')) #select columns

data12se_asvmatrix_nc_lor_nfc <- merge(sample_info,data12se_asvmatrix_nc_lor_nfc, all.y=TRUE) #merge
...
#add ASVs across PCR replicates
#aggregate data to sum ASVs across PCR replicates
data12su_asvmatrix_nc_lor_nfc <- data12su_asvmatrix_nc_lor_nfc %>%
  group_by(sample_name) %>%
  summarise(across(starts_with("ASV"), sum, na.rm = TRUE))
...
data12su_asvmatrix_nc_lor_nfc <- merge(sample_info,data12su_asvmatrix_nc_lor_nfc, all.y=TRUE) #merge
