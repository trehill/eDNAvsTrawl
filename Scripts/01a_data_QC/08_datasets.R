...
ASVtaxsample12se <- merge(ASVtaxsample12se, eDNA_meta_12se, by.x=c('sample_name'), by.y=c('original_sample_name'), all.x=TRUE)
      #increases ASVtaxsample12se observations by three because there are three replicates in eDNA 

ASVtaxsample12su <- merge(ASVtaxsample12su, eDNA_meta_12su, by.x=c('sample_name'), by.y=c('original_sample_name'), all.x=TRUE)
...
ncol(ASVtaxsample12se)
ncol(ASVtaxsample12su)

#subset only valid set numbers
...
eDNA_df <- data.frame(lapply(eDNA_df, function(x) {
  gsub("Clupea palassi", "Clupea pallasii", x)  #to match trawl dataset 

})) 

eDNA_df <- data.frame(lapply(eDNA_df, function(x) {
  gsub("Ammodytes personatus", "Ammodytes hexapterus", x)  #to match trawl dataset, theyre synonyms
...
eDNA_df <- data.frame(lapply(eDNA_df, function(x) {
  gsub("Sebastes crameri/diploproa", "Sebastes", x)  #to match trawl dataset 
  
})) 

eDNA_df <- data.frame(lapply(eDNA_df, function(x) {
  gsub("Sebastes crameri/entomelas", "Sebastes", x)  #to match trawl dataset 
