...
ggsave("./Outputs/analysis_b/biomass/biomass_box_all.png", 
       plot = plot,
...
mean(both$biomass_index) #1.1
mean(trawl_o$biomass_index) #0.0037
...
se_both$se_biomass  # Standard error for 'both eDNA/trawl' group, 0.334
se_trawl_only$se_biomass  # Standard error for 'only trawl' group, 0.00123
...
with(data, shapiro.test(biomass_index[gamma_detection_method == "both eDNA/trawl"])) #p = 8.393e-16

# Shapiro-Wilk normality test for only trawl biomass index 
with(data, shapiro.test(biomass_index[gamma_detection_method == "only trawl"])) #p=0.0008154
...
with(data, shapiro.test(biomass_index[gamma_detection_method == "both eDNA/trawl"])) #p = 0.5182
with(data, shapiro.test(biomass_index[gamma_detection_method == "only trawl"])) #p=0.04161
#now we have a normal distribution

#check variance homogeneity 
res.ftest <- var.test(biomass_index ~ gamma_detection_method, data = data) #p=0.0.1341
res.ftest
...
#t = 6.8853, df = 85, p-value= 4.662e-10
