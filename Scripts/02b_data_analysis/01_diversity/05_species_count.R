...
only_gamma <- select(df, c('LCT', 'gamma_detection_method','n')) #select only gamma species 
only_gamma <- distinct(only_gamma)

#write file to see! 
write_csv(only_gamma,
          here("Processed_data",
               "datasets",
               "diversity",
               "gamma_spp_count_B.csv")) 

#these numbers will be used to make the proportions for the spp. images in Fig. 1 (see next script in folder)
...
plot <- ggplot(df, aes(x=as.factor(gamma_detection_method), y=n)) + 
 ...
 ggsave("./Outputs/analysis_b/diversity/gamma_incidence_box.png", 
        plot = plot,
 ...
 unique(northern$LCT) #15 species total 
 
 unique(southern$LCT) #32 species total 
 ...
 unique(n_beta$LCT) #9 species total 
 unique(s_beta$LCT) #13 species total 
 ...
 unique(n_beta$LCT) #4 species total 
...
mean(overlap$overlap_perc) #29% #percentage overlap by both methods
...
mean(overlap$overlap_perc) #64% #percentage overlap by eDNA
